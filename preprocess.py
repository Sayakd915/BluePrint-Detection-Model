...
TILED_IMG_DIR = Path("tiled/images")
TILED_LBL_DIR = Path("tiled/labels")
    ...
    for y in range(0, h - TILE_SIZE + 1, step):
        for x in range(0, w - TILE_SIZE + 1, step):
            ...
            for cls, cx, cy, bw, bh in labels:
                ...
                box_right = box_left + abs_w
                box_bottom = box_top + abs_h

                if (x < box_right and x + TILE_SIZE > box_left and
                    y < box_bottom and y + TILE_SIZE > box_top):
                    new_cx = (abs_x - x) / TILE_SIZE
                    new_cy = (abs_y - y) / TILE_SIZE
                    new_w = abs_w / TILE_SIZE
                    new_h = abs_h / TILE_SIZE

                    if 0 < new_w <= 1 and 0 < new_h <= 1:
                        tile_labels.append(f"{int(cls)} {new_cx:.6f} {new_cy:.6f} {new_w:.6f} {new_h:.6f}")

            if tile_labels or INCLUDE_EMPTY_TILES:
...
def main():
    for folder in ["tiled", "split"]:
        shutil.rmtree(folder, ignore_errors=True)
    for p in [TILED_IMG_DIR, TILED_LBL_DIR]:
    ...
    for split in ['train', 'val']:
        (SPLIT_IMG_DIR / split).mkdir(parents=True, exist_ok=True)
        (SPLIT_LBL_DIR / split).mkdir(parents=True, exist_ok=True)

    for split, files in zip(['train', 'val'], [train_imgs, val_imgs]):
        for f in files:
            name = f.name
            shutil.copy(f, SPLIT_IMG_DIR / split / name)
            shutil.copy(TILED_LBL_DIR / f"{f.stem}.txt", SPLIT_LBL_DIR / split / f"{f.stem}.txt")

    with open("dataset.yaml", "w") as f:
        f.write(f"""train: split/images/train
...
""")
    ...
    print("Dataset tiled and split. `dataset.yaml` created.")
